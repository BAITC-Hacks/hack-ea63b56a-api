...
interface RecommendationResponse {
  ...
  suggestions: Array<{ type: 'budget' | 'duration' | 'language' | 'date'; label: string; value: number | string; candidateCount: number }>;
  ai: { enabled: boolean; used: boolean; model: string; feature: 'recommendation_explainer' };
}
...
const FALLBACK_METADATA: Metadata = {
  ai: { enabled: false, model: 'gpt-6-luna' },
...
export default function Home(): React.ReactElement {
  ...
  const [submittedForm, setSubmittedForm] = useState<SearchForm>(INITIAL_FORM);
  const [lastAttempt, setLastAttempt] = useState<{ prompt: string } | { form: SearchForm } | null>(null);
  ...
  const [parserSource, setParserSource] = useState<'openai' | 'fallback' | null>(null);
  ...
  const summary = useMemo(() => [
    ...
    submittedForm.budgetKzt && `до ${money.format(Number(submittedForm.budgetKzt))} ₸`,
    submittedForm.durationHours && `${submittedForm.durationHours} ч.`,
    submittedForm.language.split('|').filter(Boolean).join(' + ')
  ].filter(Boolean), [submittedForm]);
  ...
  const submitPrompt = async (text: string): Promise<void> => {
    ...
    try {
      ...
      setParserSource(data.source);
      const nextForm: SearchForm = {
        ...INITIAL_FORM,
        ...data.parsed,
        budgetKzt: data.parsed.budgetKzt?.toString() ?? '',
        durationHours: data.parsed.durationHours?.toString() ?? '',
        wishes: data.parsed.wishes ?? ''
      };
      setForm(nextForm);
  ...
  const applySuggestion = (suggestion: RecommendationResponse['suggestions'][number]): void => {
    ...
    if (suggestion.type === 'date') next.date = String(suggestion.value);
    setForm(next);
    void runRecommendation(next);
  };

  const editConditions = (): void => {
    setManualOpen(true);
    window.setTimeout(() => document.querySelector('.manualForm')?.scrollIntoView({ behavior: 'smooth', block: 'center' }), 50);
  };
  ...
  return (
    <main>
      ...
      <section className="hero" id="top">
        ...
        <form className="promptCard" onSubmit={(event) => { event.preventDefault(); void submitPrompt(prompt); }}>
        ...
        <button className="manualToggle" aria-expanded={manualOpen} aria-controls="manual-parameters" onClick={() => setManualOpen((open) => !open)} type="button">
          <span className="sliders">☷</span> Указать параметры вручную
          <span className={manualOpen ? 'chevron open' : 'chevron'}>⌄</span>
        </button>

        {manualOpen && (
          <form className="manualForm" id="manual-parameters" onSubmit={submitManual}>
            ...
            <div className="formGrid">
              ...
              <fieldset className="field languageField"><legend>Языки — необязательно</legend><div>{metadata.languages.map((language) => <label key={language}><input type="checkbox" checked={form.language.split('|').includes(language)} onChange={(event) => { const selected = form.language.split('|').filter(Boolean); update('language', (event.target.checked ? [...selected, language] : selected.filter((item) => item !== language)).join('|')); }} />{language}</label>)}</div></fieldset>
      ...
      {error && (
        <section className="errorState" role="alert">
          <span>!</span><div><h2>Не удалось выполнить подбор</h2><p>Не удалось связаться с сервисом. Попробуйте ещё раз или измените условия.</p></div>
          <button disabled={loading || !lastAttempt} onClick={() => { if (lastAttempt) { if ('prompt' in lastAttempt) void submitPrompt(lastAttempt.prompt); else void runRecommendation(lastAttempt.form); } }} type="button">Повторить</button>
          <button onClick={editConditions} type="button">Изменить условия</button>
        </section>
      )}
      {result && <Results form={submittedForm} parserSource={parserSource} result={result} onEdit={editConditions} onSuggestion={applySuggestion} summary={summary} />}
  ...
  return (
    <section className="results" id="results">
      ...
      <div className={result.ai.used || parserSource === 'openai' ? 'aiStatus active' : 'aiStatus'}>
        <SparkIcon />
        {result.ai.used || parserSource === 'openai'
          ? `OpenAI ${result.ai.model} использован для ${result.ai.used && parserSource === 'openai' ? 'разбора и объяснений' : result.ai.used ? 'объяснений' : 'разбора запроса'}`
          : 'Подбор выполнен по каталогу без AI-обработки'}
      </div>
  ...
  return (
    <article className="contractorCard">
      ...
      <div className="cardBody">
        ...
        <div className="traits"><span>{item.languages.join(' · ')}</span><span>{item.maxHours ? `до ${item.maxHours} часов` : 'без привязки к часам присутствия'}</span><span>{item.eventFormats.slice(0, 3).join(' · ')}</span></div>
