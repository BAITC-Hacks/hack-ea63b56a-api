...
import { ArrowRight, BadgeCheck, CircleAlert, Clock3, Coins, Info, LoaderCircle, MapPin, Search, SlidersHorizontal, Sparkles } from "lucide-react";
...
const fieldClass = "mt-2 h-12 w-full rounded-md border border-line bg-white px-4 text-sm font-medium text-ink transition-colors placeholder:text-muted/70 hover:border-field-hover focus:border-teal focus:ring-2 focus:ring-teal/15 disabled:cursor-not-allowed disabled:bg-canvas";
...
function Brand() {
  return <div className="flex items-center gap-3" aria-label="HackAlem">
    ...
    <span className="text-lg font-extrabold">HackAlem<span className="text-teal">.</span></span>
  </div>;
}
...
function Results({ result, request }: { result: RecommendationResponse; request: RecommendationRequest }) {
  ...
  return <section className="mt-9 border-t border-line pt-8" aria-labelledby="results-title" aria-live="polite">
    <div className="flex flex-wrap items-start justify-between gap-3">
      <div>
        <p className="mb-2 text-xs font-extrabold uppercase text-teal">Результат подбора</p>
        <h2 id="results-title" className="text-xl font-extrabold sm:text-2xl">{title}</h2>
    ...
    <div className={`mt-5 rounded-md border px-4 py-3.5 text-sm leading-6 ${result.status === "matched" ? "border-success-line bg-success-bg text-success-text" : "border-warning-line bg-warning-bg text-warning-text"}`}>
      <p className="font-semibold">{result.message}</p>
    </div>
    {result.items.length > 0 && <ol className="mt-5 space-y-3">
      {result.items.slice(0, 3).map((item, index) => <li key={item.id} className="rounded-md border border-line bg-white p-5 sm:p-6">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="flex min-w-0 items-start gap-3">
            <span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-teal-soft text-xs font-extrabold text-teal">{String(index + 1).padStart(2, "0")}</span>
            <div>
        ...
        <p className="mt-4 max-w-3xl text-sm leading-6 text-ink/85">{item.explanation}</p>
        <div className="mt-4 flex flex-wrap gap-2 text-xs font-semibold">
          <span className={`rounded-md px-2.5 py-1 ${item.synthetic ? "bg-synthetic-bg text-synthetic-text" : "bg-source-bg text-source-text"}`}>{item.synthetic ? "Синтетический профиль" : "Реальный профиль (анонимизирован)"}</span>
          {item.city_imputed && <span className="rounded-md bg-flag-bg px-2.5 py-1 text-muted">Город добавлен при подготовке</span>}
          {item.price_imputed && <span className="rounded-md bg-flag-bg px-2.5 py-1 text-muted">Цена добавлена при подготовке</span>}
        </div>
      </li>)}
    </ol>}
  ...
  return <main className="min-h-screen">
    ...
    <div className="mx-auto max-w-6xl px-5 pb-20 pt-9 sm:px-8 sm:pt-14">
      <div className="mb-8 max-w-3xl sm:mb-10">
        <p className="mb-3 inline-flex items-center gap-2 text-xs font-extrabold uppercase text-teal"><span className="h-1.5 w-1.5 rounded-full bg-teal" /> Подбор по вашему запросу</p>
        <h1 className="text-3xl font-extrabold leading-tight sm:text-4xl">Найдите подрядчика для мероприятия</h1>
      ...
      <div className="grid items-start gap-8 lg:grid-cols-[minmax(0,1fr)_260px] lg:gap-10">
        <div className="min-w-0 border-t border-line pt-6">
          <div className="mb-7 flex items-center gap-3 border-b border-line pb-5"><span className="grid h-9 w-9 place-items-center rounded-lg bg-teal-soft text-teal"><SlidersHorizontal size={19} /></span><div><h2 className="font-extrabold">Параметры события</h2><p className="mt-0.5 text-xs text-muted">Поля со звёздочкой обязательны</p></div></div>
          ...
          {catalog.isError && <div role="alert" className="mb-5 rounded-md border border-warning-line bg-warning-bg p-4 text-sm text-warning-text">Не удалось загрузить справочник. <button type="button" onClick={() => catalog.refetch()} className="font-bold underline underline-offset-2">Повторить</button></div>}
          <form onSubmit={handleSubmit(submit)} onChange={() => setSelectedPreset(null)} noValidate>
            <div className="grid gap-x-5 gap-y-5 sm:grid-cols-2">
              ...
              <Field id="date" label="Дата мероприятия *" error={errors.date?.message}>
                <input id="date" type="date" min={choices.calendar.from} max={choices.calendar.to} disabled={!catalog.data} className={fieldClass} aria-invalid={!!errors.date} aria-describedby={errors.date ? "date-error" : undefined} {...register("date")} />
              </Field>
            ...
            <div className="mt-8 flex flex-wrap items-center gap-4 border-t border-line pt-6"><button type="submit" disabled={!catalog.data || recommendation.isPending} className="inline-flex h-12 items-center justify-center gap-2 rounded-md bg-teal px-6 text-sm font-extrabold text-white transition-colors hover:bg-teal-dark disabled:cursor-not-allowed disabled:opacity-55">{recommendation.isPending ? <><LoaderCircle size={17} className="animate-spin" /> Подбираем…</> : <><Search size={17} /> Найти подрядчиков <ArrowRight size={17} /></>}</button><p className="text-xs leading-5 text-muted">Доступные даты: {dateLabel.format(new Date(`${choices.calendar.from}T00:00:00Z`))} — {dateLabel.format(new Date(`${choices.calendar.to}T00:00:00Z`))}</p></div>
          </form>
          {recommendation.isError && <div role="alert" className="mt-6 flex gap-3 rounded-md border border-warning-line bg-warning-bg p-4 text-sm text-warning-text"><CircleAlert size={18} className="mt-0.5 shrink-0" /><p>{recommendation.error.message}</p></div>}
        ...
        <aside className="lg:pt-1" aria-label="Примеры запросов">
          <div className="mb-4"><p className="text-xs font-extrabold uppercase text-teal">Быстрый старт</p><h2 className="mt-2 text-lg font-extrabold">Попробуйте пример</h2><p className="mt-1 text-sm leading-6 text-muted">Примеры заполнят форму и запустят подбор.</p></div>
          <div className="overflow-hidden rounded-md border border-line bg-white">
            {presets.map((preset) => <button key={preset.id} type="button" disabled={!catalog.data || recommendation.isPending} onClick={() => choosePreset(preset)} aria-pressed={selectedPreset === preset.id} className="group flex w-full items-center justify-between gap-3 border-b border-line px-4 py-4 text-left last:border-b-0 hover:bg-canvas disabled:cursor-not-allowed disabled:opacity-55 aria-pressed:bg-selected-bg"><span><span className="block text-sm font-bold text-ink">{preset.title}</span><span className="mt-1 block text-xs text-muted">{preset.description}</span></span><ArrowRight size={16} className="shrink-0 text-teal transition-transform group-hover:translate-x-0.5" /></button>)}
          </div>
          <div className="mt-6 border-t border-line pt-5 text-xs leading-5 text-muted"><p className="flex items-start gap-2"><Info size={16} className="mt-0.5 shrink-0 text-amber" /> Цены указаны «от». Синтетические профили и данные, добавленные при подготовке, отмечены в результатах.</p><p className="mt-3 flex items-start gap-2"><BadgeCheck size={16} className="mt-0.5 shrink-0 text-teal" /> Занятые на выбранную дату подрядчики исключаются из выдачи.</p></div>
        </aside>
      </div>
    </div>
  </main>;
