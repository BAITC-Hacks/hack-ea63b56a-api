...
import {
  ...
  MicVocal, Music2, Search, ShieldCheck, SlidersHorizontal, Sparkles, UsersRound,
} from "lucide-react";
...
function Brand() {
  return <div className="flex items-center gap-3" aria-label="HackAlem">
    <Image src="/brand-mark.svg" width={38} height={38} alt="" aria-hidden="true" priority />
    <div className="leading-none">
      <span className="block text-lg font-extrabold text-foreground">HackAlem<span className="text-primary">.</span></span>
      <span className="mt-1 hidden text-[11px] font-semibold text-muted-foreground sm:block">Contractor Match</span>
    </div>
  </div>;
}
...
function SelectField({ id, name, label, options, placeholder, error, disabled, control, onManualChange }: {
  id: string;
  name: SelectName;
  label: string;
  options: string[];
...
function Results({ result, request }: { result: RecommendationResponse; request: RecommendationRequest }) {
  ...
  return <section aria-labelledby="results-title" aria-live="polite" className="min-w-0">
    ...
    <Alert className={result.status === "matched" ? "border-[#b8dfd3] bg-[#eef8f4] text-[#215f50]" : "border-[#e6d3ae] bg-[#fff8e9] text-[#76521f]"}>
      {result.status === "matched" ? <CheckCircle2 /> : <Info />}
      <AlertTitle>{result.status === "matched" ? "Подбор завершён" : "Результат по заданным условиям"}</AlertTitle>
    ...
    {result.totalCandidates > 0 && <div className="mt-5 rounded-lg border border-border bg-white px-4 py-3 text-xs leading-5 text-muted-foreground">
      <p><strong className="text-foreground">Воронка:</strong> в городе и категории {result.totalCandidates}, прошли условия {result.eligibleCount}, показано {result.count}.</p>
  ...
  return <main className="min-h-screen bg-background">
    <header className="border-b border-border bg-white">
      <div className="mx-auto flex max-w-screen-2xl items-center justify-between gap-3 px-4 py-3 sm:px-6 lg:px-8">
        <Brand />
        <Badge variant="outline" className="border-primary/20 bg-accent text-accent-foreground"><Sparkles />AI-рекомендации</Badge>
      </div>
    </header>
            ...
            <CardContent className="space-y-5 pt-5">
              ...
              {!catalog.isPending && <>
                <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-1">
                  <SelectField id="city" name="city" label="Город *" options={choices.cities} placeholder="Выберите город" disabled={!catalog.data} error={errors.city?.message} control={control} onManualChange={clearPreset} />
                  <Field id="date" label="Дата мероприятия *" error={errors.date?.message}>
                    <Input id="date" type="date" min={choices.calendar.from} max={choices.calendar.to} disabled={!catalog.data} aria-invalid={!!errors.date} aria-describedby={errors.date ? "date-error" : undefined} {...register("date")} />
                  </Field>
                </div>
                <Separator />
                <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-1">
                  <SelectField id="eventFormat" name="eventFormat" label="Формат мероприятия *" options={choices.eventFormats} placeholder="Выберите формат" disabled={!catalog.data} error={errors.eventFormat?.message} control={control} onManualChange={clearPreset} />
                  <SelectField id="category" name="category" label="Категория подрядчика *" options={choices.categories} placeholder="Выберите категорию" disabled={!catalog.data} error={errors.category?.message} control={control} onManualChange={clearPreset} />
                </div>
                <Separator />
                <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-1">
                  <Field id="budgetKzt" label="Бюджет, ₸ *" hint="Максимальная стартовая цена" error={errors.budgetKzt?.message}>
                    <div className="relative"><Input id="budgetKzt" type="number" min="1" step="1" inputMode="numeric" placeholder="Например, 900 000" className="pr-10" aria-invalid={!!errors.budgetKzt} aria-describedby={errors.budgetKzt ? "budgetKzt-error" : undefined} {...register("budgetKzt", { valueAsNumber: true })} /><Coins className="pointer-events-none absolute right-3 top-3.5 h-4 w-4 text-muted-foreground" /></div>
                  </Field>
                  <SelectField id="language" name="language" label="Язык" options={choices.languages} placeholder="Любой язык" disabled={!catalog.data} control={control} onManualChange={clearPreset} />
                  <Field id="durationHours" label="Длительность, часы" hint="Необязательно, максимум 24 часа" error={errors.durationHours?.message}>
                    <div className="relative"><Input id="durationHours" type="number" min="0.1" max="24" step="0.5" inputMode="decimal" placeholder="Необязательно" className="pr-10" aria-invalid={!!errors.durationHours} aria-describedby={errors.durationHours ? "durationHours-error" : undefined} {...register("durationHours", { setValueAs: (value: string) => value === "" ? undefined : Number(value) })} /><Clock3 className="pointer-events-none absolute right-3 top-3.5 h-4 w-4 text-muted-foreground" /></div>
                  </Field>
                </div>
              </>}
            </CardContent>
